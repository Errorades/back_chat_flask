$(document).ready(function () {
    ...
    function encryptMessage(message, sharedKey) {
        var encryptedMessage = CryptoJS.AES.encrypt(JSON.stringify(message), sharedKey).toString();
        return encryptedMessage;
    }

    function decryptMessage(encryptedMessage, sharedKey) {
        var decryptedBytes = CryptoJS.AES.decrypt(encryptedMessage, sharedKey);
        var decryptedMessage = JSON.parse(decryptedBytes.toString(CryptoJS.enc.Utf8));
    ...
    chatBox.on('click', '#sendButton', function () {
        ...
        socket.emit('send_msg', { 'chat_id': chatId, 'message': encryptedMessage });
        messageInput.val('');
    });
